...
update_panel() {
  ...
  if [[ -n "$DB_INFO" ]]; then
    ...
    if [[ -f ".env" ]]; then
      echo "🔄 尝试从 .env 文件读取配置..."
      DB_NAME=$(grep "^DB_NAME=" .env_err | cut -d'=' -f2 2>/dev/null)
      DB_PASSWORD=$(grep "^DB_PASSWORD=" .env_err | cut -d'=' -f2 2>/dev/null)
      DB_USER=$(grep "^DB_USER=" .env_err | cut -d'=' -f2 2>/dev/null)
...
install_reverse_proxy() {
  ...
  FRONTEND_HINT=""
  if [[ -f ".env" ]]; then
    ENV_FRONTEND_PORT=$(grep "^FRONTEND_PORT=" .env_err | cut -d'=' -f2 2>/dev/null || echo "")
...
uninstall_panel() {
  ...
  echo "🧹 删除配置文件..."
  #rm -f docker-compose.yml gost.sql .env_err
  echo "✅ 卸载完成"
}
